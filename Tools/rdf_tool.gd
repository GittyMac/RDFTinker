...
"""
This RDF tool was adapted from an existing Python port.
License is in the NOTICE file.
https://github.com/JakeSESaunders/py-rdf
"""
...
static var start = [0xFF, 0x00, 0xFF, 0xAC, 0xEB, 0x96, 0xC4, 0x2A]

static func decode_bytes(rdf_data):
	var current_column = 0
	var pos = 0
	var end_char = 0
	var decoded_bytes = PackedByteArray()
	for char in rdf_data:
		for i in range(256):
			if column[current_column][i] == char:
				decoded_bytes.append(code[i])
				pos += 1
				if code[i] == 0x3E:
					end_char = pos
				break
		...
		if current_column > 7:
			current_column = 0
	return decoded_bytes.slice(len(start), end_char).get_string_from_ascii()

static func encode_bytes(xml_data):
	var current_column = 0
	var pos = 0
	var encoded_bytes = PackedByteArray()
	for char in xml_data:
		for i in range(256):
			if char(code[i]) == char:
				encoded_bytes.append(column[current_column][i])
				break
		current_column += 1
		if current_column > 7:
			current_column = 0
	var start_bytes = PackedByteArray(start)
	var end_bytes = PackedByteArray(column_endings[current_column])
	return start_bytes + encoded_bytes + end_bytes
